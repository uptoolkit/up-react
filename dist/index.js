    ...
    module2.exports = function httpAdapter(config2) {
      return new Promise(function dispatchHttpRequest(resolvePromise, rejectPromise) {
        ...
        if (config2.transport) {
          ...
          transport = isHttpsProxy ? https : http2;
        } else {
...
var import_js_config_helper = require("js-config-helper");
var import_axios = __toESM(require_axios2());
var import_js_form_helper = require("js-form-helper");
...
var exported;
var UpInit = false;
var setUp = async function setUp2(options) {
  config = new import_js_config_helper.Config(options);
  http = config.get("override.http") || import_axios.default.create();
  api = config.get("override") || import_axios.default.create({
    baseURL: config.get("api.url")
  });
  if (!config.has("exclude.form")) {
    form = function(data, options2) {
      return new import_js_form_helper.Form(data, {
        ...{
          http
        },
        ...options2
  ...
  if (!config.has("exclude.i18n")) {
    if (options.i18n) {
      await import_i18next.default.use(import_react_i18next.initReactI18next).init(options.i18n);
    }
  }
  exported = {
    config,
...
var UpContext = (0, import_react.createContext)({});
function UpProvider({ options, children }) {
  const [loading, setLoading] = (0, import_react.useState)(true);
  const [up, setInitUp] = (0, import_react.useState)();
  const initUp = async () => {
    const up2 = await setUp({
      ...options,
      loading,
      setLoading
    });
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(UpContext.Provider, { value: up, children });
}
